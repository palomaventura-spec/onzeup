...
export default async function ClubPlanStatusCard({
  ...
  const currentPlan = planLabel(subscription?.plan);

  return (
    <section
      className={`card clean-site-card${isComplimentary ? " dashboard-complimentary-card" : ""}`}
          ...
          <>
            ...
            <p className="muted">
              ...
              {organization?.complimentaryUntil
                ...
                : " sem prazo definido"}
              . Sem cobrança durante a cortesia.
            </p>
            <div className="dashboard-access-meta">
              <span className="dashboard-access-badge">Cortesia</span>
              {organization?.complimentaryReason ? (
                <span className="dashboard-access-note">
                  {organization.complimentaryReason}
                </span>
              ) : null}
            </div>
