...
export default async function CoachEdit({searchParams}:{searchParams:Promise<{salvo?:string}>}){
 ...
 return <main className="coach-editor">
  ...
  <form action={saveCoach} className="card coach-form stack">
   <div className="form-grid-2">
    ...
    <label>Cidade<input name="city" defaultValue={p.city||""}/></label><label>Estado<input name="state" defaultValue={p.state||""}/></label>
    <label>País<input name="country" defaultValue={p.country||""}/></label><label>Nacionalidade<input name="nationality" defaultValue={p.nationality||""}/></label>
   </div>
   ...
   <label className="check-row"><input name="isPublic" type="checkbox" defaultChecked={p.isPublic}/><span>Publicar meu perfil</span></label>
