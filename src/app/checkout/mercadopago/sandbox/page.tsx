...
export default async function MercadoPagoSandboxPage({
  ...
  if (!mercadoPagoIsTestMode()) redirect("/responsavel");
    ...
    return (
      <main className="auth-shell">
        <section className="auth-card">
          ...
          <p>Configure MERCADOPAGO_PUBLIC_KEY com a Public Key TEST da mesma aplicação que fornece o Access Token TEST.</p>
          <Link className="btn" href="/responsavel">Voltar</Link>
        </section>
      </main>
    );
  }
    ...
    <main className="auth-shell mp-sandbox-shell">
      <section className="auth-card mp-sandbox-card">
        ...
        <div className="mp-product-summary">
          <div><strong>{player.nickname || player.name}</strong><span>ONZEUP Player Premium</span></div>
          <b>R$ 29,90/mês</b>
        </div>

        <div className="mp-test-warning">
          <strong>Teste técnico — nenhum cartão real.</strong>
          <p>Use somente um cartão oficial de teste do Mercado Pago.</p>
        </div>

        <MercadoPagoSandboxCardForm publicKey={publicKey} playerId={player.id} payerEmail={payerEmail} />
        <Link className="btn-secondary" href={`/responsavel?player=${player.id}`}>Cancelar e voltar</Link>
      </section>
    </main>
  );
}
