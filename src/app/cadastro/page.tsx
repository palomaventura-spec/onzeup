...
export default async function Register({searchParams}:{searchParams:Promise<{status?:string;erro?:string}>}){
 const q=await searchParams;
 return <main className="auth-marketing-page">
   <section className="auth-product-copy"><Link href="/" className="marketing-brand">ONZE<span>UP</span></Link><span className="marketing-kicker">ONZE PLAYER FREE</span><h1>Crie a identidade<br/>esportiva do atleta.</h1><p>Perfil público, dados esportivos, estatísticas, trajetória, um vídeo do YouTube e endereço próprio na ONZEUP.</p><strong>R$ 0 • sem cartão</strong></section>
   <section className="auth-form-card"><span className="page-eyebrow">CONTA DO RESPONSÁVEL</span><h2>Criar ONZE Player grátis</h2>
