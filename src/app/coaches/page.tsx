...
export default async function CoachesHome({searchParams}:{searchParams:Promise<{q?:string;role?:string;location?:string}>}){
 ...
 return <main className="catalog-first-page coach-catalog-page">
   <header className="catalog-nav"><a href="/coaches" className="player-brand">11<span>UP</span> <b>COACHES</b></a><nav><a href="#todos">Profissionais</a><a href="https://onzeup.com.br/coach">Sobre o Coach</a><a href="https://onzeup.com.br/login?perfil=coach">Entrar</a><a className="btn" href="/cadastro-coach">Criar perfil grátis</a></nav></header>
   <section className="catalog-title"><span className="page-eyebrow">11UP COACHES</span><h1>Profissionais do futebol.</h1><p>Treinadores, auxiliares, scouts e profissionais que desenvolvem atletas e projetos.</p></section>
   {featured.length?<section className="catalog-featured"><div className="catalog-section-title"><div><span className="page-eyebrow">SELEÇÃO 11UP</span><h2>Profissionais em destaque</h2></div></div><div className="featured-coach-grid">{featured.map(c=><a href={`/coach-profile/${c.slug}`} key={c.id} className="featured-coach-card"><div className="catalog-photo">{c.photoUrl?<img src={c.photoUrl} alt={c.name}/>:<span>{c.name.slice(0,2)}</span>}</div><span>★ DESTAQUE 11UP</span><h3>{c.professionalName||c.name}</h3><p>{[c.roleTitle,c.currentClub,c.city].filter(Boolean).join(" • ")}</p><strong>Ver perfil →</strong></a>)}</div></section>:null}
   <section className="catalog-search-zone" id="todos"><div className="catalog-section-title"><div><span className="page-eyebrow">PESQUISA</span><h2>Encontre um profissional</h2></div><span className="badge">{coaches.length} perfil(is)</span></div>
    ...
    <div className="catalog-grid">{coaches.map(c=><a href={`/coach-profile/${c.slug}`} className="catalog-person-card coach" key={c.id}><div className="catalog-photo">{c.photoUrl?<img src={c.photoUrl} alt={c.name}/>:<span>{c.name.slice(0,2).toUpperCase()}</span>}</div><small>11UP COACH</small><h3>{c.professionalName||c.name}</h3><p>{[c.roleTitle,c.currentClub,c.city].filter(Boolean).join(" • ")}</p><strong>Ver perfil →</strong></a>)}{!coaches.length?<div className="catalog-empty"><h3>Nenhum profissional encontrado.</h3><p>Crie seu perfil gratuito e seja um dos primeiros Coaches da plataforma.</p></div>:null}</div>
   </section>
   <section className="catalog-conversion"><div><span className="page-eyebrow">11UP COACH • 100% GRATUITO</span><h2>Construa sua presença profissional.</h2><p>Crie seu perfil e use seu link de parceiro para indicar o 11UP Player às famílias.</p></div><div><a className="btn" href="/cadastro-coach">Criar Coach grátis</a><a className="players-secondary-cta" href="https://onzeup.com.br/coach">Como funciona a parceria →</a></div></section>
 </main>
}
