      ...
      callbacks: {
        ...
        onSubmit: async (event: Event) => {
          ...
          try {
            ...
            const email = String(data.cardholderEmail || "").trim();

            if (!cardTokenId) {
              throw new Error(
                "O Mercado Pago não gerou o token do cartão. Confira os dados de teste.",
              );
            }

            if (!email || !email.includes("@")) {
              throw new Error("Informe um e-mail válido para o pagador.");
            }

            const response = await fetch(
              "/api/mercadopago/sandbox-subscription",
              {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
              throw new Error(
                result?.error || "Não foi possível criar a assinatura.",
              );
            }

            setStatus(
              `Assinatura criada (${result.subscriptionStatus}). Aguardando confirmação do pagamento.`,
            );

            window.location.href =
              result.redirectUrl ||
              `/checkout/mercadopago/retorno?paymentId=${encodeURIComponent(
                result.paymentId,
              )}`;
          } catch (error) {
            ...
            setStatus(
              error instanceof Error
                ? error.message
                : "Erro ao criar assinatura de teste.",
            );
            setBusy(false);
          }
        },
  ...
  return (
    <>
      ...
      <form id="form-checkout" className="mp-sandbox-form">
        <div className="mp-sandbox-grid">
          ...
          <label className="mp-field mp-field-wide">
            ...
            <input
              ...
              placeholder="Use um e-mail comum diferente da sua conta Mercado Pago"
              autoComplete="off"
            />
            <small className="field-help">
              Não use o e-mail da sua conta Mercado Pago e não use e-mail de
              usuário de teste do Mercado Pago.
            </small>
          </label>
        </div>
        ...
        <progress
          value="0"
          className="progress-bar mp-progress"
