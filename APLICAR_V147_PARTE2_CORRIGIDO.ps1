...
Write-Host "ONZEUP v1.4.7 - Commercial Release Parte 2 FIX" -ForegroundColor Cyan
...
if (Test-Path $responsavel) {
  ...
  if ($c -notmatch 'createPlayerPremiumPixAsaas') {
    Write-Host "Aviso: import Pix nao foi encontrado para substituicao."
  }

  if ($c -notmatch 'Pagar mensalidade via Pix') {
    $needle = '<button className="btn">Assinar Premium</button>'
    $replacement = '<button className="btn">Assinar com cartao</button></form><form action={createPlayerPremiumPixAsaas}><input type="hidden" name="playerId" value={selected.id} /><button className="btn-secondary">Pagar mensalidade via Pix</button>'
  ...
  WriteFile $responsavel $c
  Write-Host "Atualizado: Player cartao + Pix"
}

# HOME - apenas substituicoes simples, sem setas nem acentos problemáticos
